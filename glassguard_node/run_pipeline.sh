#!/bin/bash
# 3-node GlassGuard pipeline (throughput build). Launches the provider + the 3 stage
# nodes as SEPARATE processes (3 GILs -> true multicore). The single-process node
# (../run_glassguard.sh) is unchanged and remains the scoring reference.
#
...
# preflight: the stage bodies must be ported out of ../glassguard_ros_node.py _process
...
if grep -q "raise NotImplementedError" "$PIPE"/node1_detect.py "$PIPE"/node2_geometry.py "$PIPE"/node3_mapping.py; then
  ...
  echo "             cd $GK && METHOD=360 BAG=$BAG ./run_glassguard.sh"
  exit 2
fi
...
( source "$ROS_SETUP"; [ -f "$CAM_INSTALL" ] && source "$CAM_INSTALL"
  exec ros2 launch extrinsic_latency_calib glassguard.launch ) > /tmp/pipe_provider.log 2>&1 &
sleep 3
( source "$ROS_SETUP"
...
( source "$ROS_SETUP"; exec ros2 run rviz2 rviz2 -d "$GK/glassguard.rviz" ) > /tmp/pipe_rviz.log 2>&1 &
sleep 2
( source "$ROS_SETUP"
